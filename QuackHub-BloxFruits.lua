...
local function FastAttack()
    pcall(function()
        local CombatFramework = ReplicatedStorage:FindFirstChild("CombatFramework")
        if CombatFramework then
            local CFS = require(CombatFramework)
            if CFS and CFS.activeController then
                local atk = CFS.activeController
                atk.hitboxMagnitude = 200
                pcall(function()
                    atk:Attack()
                end)
            end
        else
            -- Fallback: use melee hit system
...
-- Now create the bot panel toggles (after function is defined)
